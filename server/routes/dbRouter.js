...
/* GET db/logs/len */
dbRouter.get('/logs/len/:start/:end', async (req, res, next) => {
  try {
    ...
    if (start !== 0 && end !== 0) {
      ...
      }).sort({ no: -1 }).limit(1).exec();
      res.json(log.length > 0 ? log[0].no : 0);
    }
    else {
      const log = await Log.find({}).sort({ no: -1 }).limit(1).exec();
      res.json(log.length > 0 ? log[0].no : 0);
    }
  }
  catch (error) {
...
/* GET db/logs/:counter */
dbRouter.get('/logs/:counter/:start/:end', async (req, res, next) => {
  try {
    const start = Number(req.params.start);
    const end = Number(req.params.end);
    if (start !== 0 && end !== 0){
      const logs = await Log.find({
        $and: [
          { no: { $lte: req.params.counter } },
          { sec: { $gte: start } },
          { sec: { $lte: end } }
        ]
      }).sort({ no: -1 }).limit(100).exec();
      res.json(logs);
    }
    else{
      const logs = await Log.find({ no: { $lte: req.params.counter } }).sort({ no: -1 }).limit(100).exec();
      res.json(logs);
    }
  }
  catch (error) {
  ...
  /*
  Log.find({no: {$lte: req.params.counter}}, (error, logs) => {
    if(error) next(error);
    res.json(logs);
  });*/
});
    ...
    for (let id of ids) {
      ...
      for (let date of dateList) {
        ...
        if (extractedEnterLogs.length !== 0 && extractedExitLogs.length !== 0) {
          ...
          if(enterStamp.result === 11) manual.start = true;
          ...
          if(exitStamp.result === 11) manual.end = true;
