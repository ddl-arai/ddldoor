...
import { CardRegisterComponent } from '../dialogs/card-register/card-register.component';
import { CardEditComponent } from '../dialogs/card-edit/card-edit.component';
import { CardDeleteComponent } from '../dialogs/card-delete/card-delete.component';
...
export class NfcComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	onCardRegister(): void {
		let dialogRef = this.dialog.open(CardRegisterComponent, {
			width: '400px',
	...
	onEdit(idm: string): void {
		let dialogRef = this.dialog.open(CardEditComponent, {
			width: '400px',
			data: idm
		});
		dialogRef.afterClosed().subscribe(() => {
	...
	onDelete(idm: number): void {
		let dialogRef = this.dialog.open(CardDeleteComponent, {
		  width: '400px',
		  data: idm
		});
		dialogRef.afterClosed().subscribe(() => {
